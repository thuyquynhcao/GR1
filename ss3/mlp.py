...
# Phương pháp đánh giá hệ thống phân lớp này là 'accuracy',
# tức số lượng điểm được phân loại đúng trong toàn bộ số điểm.
...
class MLP:
  ...
  # chọn thuật toán để tối ưu hàm loss
...
def train_MLP(vocab_size, predicted_labels, loss, train_op):
	train_data_reader = DataReader(
		data_path='../datasets/tf-idf/20news-train-tfidf.txt',
		batch_size=50,
		vocab_size=vocab_size
	)
	num_epoch = 0
	...
	with tf.Session() as sess:
		...
		while step < MAX_STEP:
			...
			num_epoch = train_data_reader._num_epoch
	return num_epoch

# Đánh giá model trên test data
def evaluate_MLP(num_epoch, predicted_labels):
	test_data_reader = DataReader(
		data_path='../datasets/tf-idf/20news-test-tfidf.txt',
		batch_size=50,
		vocab_size=vocab_size
	)
	accuracy_lists = []
